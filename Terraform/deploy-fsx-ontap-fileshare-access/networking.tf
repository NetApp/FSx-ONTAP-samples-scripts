...
resource "aws_security_group" "sg-fsx" {
  name        = "${var.creator_tag}-FSxN"
  description = "Allow outbound and inbound access for FSxN"
  vpc_id      = aws_vpc.vpc.id
...
resource "aws_security_group_rule" "AllVPCTrafficRule" {
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "tcp"
...
resource "aws_security_group" "sg-AD-Server" {
  name        = "${var.creator_tag}-AD"
  description = "Allow AD and DNS ports from within VPC"
  vpc_id      = aws_vpc.vpc.id

  dynamic "ingress" {
    for_each = var.ad_security_group_ingress_config
    content {
      description      = ingress.value["description"]
      from_port        = ingress.value["port"]
      to_port          = ingress.value["port"]
