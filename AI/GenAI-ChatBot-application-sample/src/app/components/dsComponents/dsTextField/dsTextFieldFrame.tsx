...
import ShowIcon from "@/app/svgs/show.svg";
import HideIcon from "@/app/svgs/hide.svg";
...
const DsTextFieldFrame = forwardRef<HTMLDivElement, DsTextFieldFrameProps>(({
    ...
    return (
        <Popover className={`dsTextFramePopover ${className}`} placement="bottom-start" trigger={!isDisabled || !disabledReason ? null : "hover"} container={<div className={`dsTextFieldFrame ${variant} ${isDisabled ? 'isDisabled' : ''} ${isLoading ? 'isLoading' : ''} ${isReadOnly ? 'isReadOnly' : ''} ${message?.type === 'error' ? 'isError' : ''}`}
            style={style}>
            ...
            <DsTypography variant="Regular_14">{disabledReason}</DsTypography>
        </Popover>
    )
})
